(function(){
  ...
  if(pform){
    ...
    next.addEventListener('click',function(){
      if(step===1){var n=pform.name.value.trim(),em=pform.email.value.trim();if(!n)return warn('Please add your name.',pform.name);if(!em||em.indexOf('@')<0)return warn('Please add a working email address so we can send the proposal.',pform.email);show(2,true)}
        ...
        window.location.href='mailto:hello@noviqstudios.nl?subject='+encodeURIComponent('Proposal request — '+(f.business.value||f.name.value))+'&body='+encodeURIComponent(body);show(4,true)}
    });
  ...
  (function(){
    ...
    function localAnswer(q){for(var i=0;i<LOCAL.length;i++){if(LOCAL[i][0].test(q))return LOCAL[i][1]}return 'I\'m not sure about that one. The quickest way to get a proper answer is the proposal form on this page, or email hello@noviqstudios.nl — we reply within one business day.'}
