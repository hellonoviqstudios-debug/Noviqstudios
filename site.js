(function(){
  ...
  if(pform){
    ...
    function show(n,scroll){step=n;panes.forEach(function(p){p.classList.toggle('on',Number(p.dataset.step)===n)});
      ...
      if(n===3)fillReview();
      if(scroll){pform.scrollIntoView({behavior:reduce?'auto':'smooth',block:'start'});
    ...
    /* preselect a maintenance plan from a ?plan= link (the Monitoring & Maintenance cards) */
    try{
      var planKey=new URLSearchParams(window.location.search).get('plan');
      var PLAN_LABELS={monitor:'Monitor — €19/month excl. 21% VAT, monthly check',protect:'Protect — €39/month excl. 21% VAT, weekly check',priority:'Priority — €69/month excl. 21% VAT, multiple checks per week'};
      if(planKey&&PLAN_LABELS[planKey]){
        var maintChk=$('f-need-maint');
        if(maintChk)maintChk.checked=true;
        if(!pform.message.value)pform.message.value='I\'m interested in the following maintenance subscription: '+PLAN_LABELS[planKey]+'.';
      }
    }catch(e){}
  ...
  /* ---------- chat agent ---------- */
  (function(){
    var KB='You are the assistant on the website of Noviq Studios, a small web design studio in the Netherlands (noviqstudios.nl, hellonoviqstudios@gmail.com, Mon–Fri 9:00–18:00 CET, works with clients anywhere in English or Dutch). Tagline: Websites that work.\n'
    ...
    +'PRICING (design & build projects): there is no price list. Each project gets a written proposal with a fixed price within two business days after the client fills in the proposal form on the site (page proposal.html, three short steps). Never invent prices or amounts for a design/build project.\n'
    +'MAINTENANCE SUBSCRIPTIONS (Website Monitoring & Maintenance, on the Services page): these ARE fixed, published prices — this is the one exception to "no price list". They exist to keep an already-launched site working, NOT for content changes, SEO or marketing. Noviq periodically checks the site, its functionality and connected systems; the price difference between plans is mainly how often it is checked. Monitor: €19/month excl. 21% VAT, checked once a month — for smaller sites that don\'t change often. Protect: €39/month excl. 21% VAT, checked once a week (labelled "Most chosen"), includes everything in Monitor plus uptime monitoring and quicker handling of issues — for sites that regularly get enquiries or customers. Priority: €69/month excl. 21% VAT, checked several times a week with continuous automated uptime monitoring where possible, includes everything in Protect plus priority handling and more frequent checks of key flows (contact forms, enquiries, bookings) — for businesses where the site is core to daily operations. Every plan checks things like: site availability, key pages, navigation, buttons, forms, mobile & desktop display, SSL/security certificate, and integrations/connected software, where technically possible. If an issue is found the client is informed and small technical fixes within normal maintenance are resolved; if an external platform or paid third-party service is the actual cause, Noviq diagnoses it but fixing it may need a separate quote. New features, big changes, redesigns, external software costs and work outside the existing site are quoted separately, not included. To sign up, use the proposal form and mention the plan (or use the "Choose Monitor/Protect/Priority" buttons on the Services page, which pre-fill this for the visitor).\n'
    ...
    var LOCAL=[
      [/onderhoud|monitoring|monitor\b|uptime|\bprotect\b|\bpriority\b|maintenance/i,'Our Website Monitoring & Maintenance subscriptions keep an already-live site working — not content, SEO or marketing. Monitor is €19/month (excl. 21% VAT, checked monthly), Protect is €39/month (weekly checks, most chosen, plus uptime monitoring), and Priority is €69/month (several checks a week, continuous uptime monitoring, priority handling). See the Services page for what\'s checked, or fill in the proposal form and mention the plan.'],
      [/pric|cost|how much|tarief|prijs|kost|budget/i,'There\'s no fixed price list for design & build projects — every project is different. Fill in the proposal form on this page and you\'ll get a written proposal with a fixed price within two business days, no obligation. (The Website Monitoring & Maintenance subscriptions on the Services page do have fixed monthly prices.)'],
      [/how long|timeline|weeks|duur|lang|snel/i,'About one week. We meet and agree the aim, you get a sample format within two business days, build and test runs take one to two business days, and connecting and launching takes one business day.'],
      ...
      [/start|begin|need from me|proposal|offerte|voorstel/i,'To start, fill in the proposal form (Request a proposal in the menu). You\'ll get a written proposal within two business days, then we meet — online or in person — and collect your logo and any photos.'],
      ...
      [/contact|email|mail|phone|bel/i,'You can reach us at hellonoviqstudios@gmail.com, Monday to Friday 9:00–18:00 CET — or use the proposal form on this page.'],
    ...
    function localAnswer(q){for(var i=0;i<LOCAL.length;i++){if(LOCAL[i][0].test(q))return LOCAL[i][1]}return 'I\'m not sure about that one. The quickest way to get a proper answer is the proposal form on this page, or email hellonoviqstudios@gmail.com — we reply within one business day.'}
