(function(){
  ...
  if(pform){
    ...
    /* preselect a maintenance plan from a ?plan= link (the Monitoring & Onderhoud cards) */
    try{
      var planKey=new URLSearchParams(window.location.search).get('plan');
      var PLAN_LABELS={monitor:'Monitor — €19/maand excl. 21% btw, maandelijkse controle',protect:'Protect — €39/maand excl. 21% btw, wekelijkse controle',priority:'Priority — €69/maand excl. 21% btw, meerdere controles per week'};
      if(planKey&&PLAN_LABELS[planKey]){
        var maintChk=$('f-need-maint');
        if(maintChk)maintChk.checked=true;
        if(!pform.message.value)pform.message.value='Ik ben geïnteresseerd in het volgende onderhoudsabonnement: '+PLAN_LABELS[planKey]+'.';
      }
    }catch(e){}
